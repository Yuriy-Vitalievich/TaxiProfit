...
- `settings` table for shared dashboard settings, including the monthly goal.
- `payload` JSON column to keep the current app data shape flexible.
- Realtime is enabled for these tables through the `supabase_realtime` publication, so open devices receive changes without manual reload.
